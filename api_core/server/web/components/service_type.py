...
@xenSecureV1
class MtimeWebServiceTypeHandler(tornado.web.RequestHandler,
        MongoMixin):
    ...
    async def get(self):
        ...
        try:
            ...
            if len(profile):
                ...
                Log.i('service_account', self.serviceAccountId)
                if len(app):
                    self.apiId = app[0]['apiId']
                    Log.i(self.apiId)
                    if self.apiId in [ 402020, 402022, 402021,402023]:
                        serTypeInfoQ = self.serviceType.find({})
                        serTypeInfo = []
                        async for i in serTypeInfoQ:
                            serTypeInfo.append(i)
                        for res in serTypeInfo:
                            v = {
                                    '_id':str(res['_id']),
                                    'name':res['name'],
                                    'active': False,
                                    #'description':res['description'],
                                    'serviceType': int(res['serviceType'])
                                }
                            serAcc = []
                            ...
                            if len(serAcc):
                                ...
                                v["active"] = False
                            result.append(v)
                            result = sorted(result, key=lambda x: x['serviceType'], reverse=False)
                        code = 2000
                        status = True
                    else:
                        code = 4003
