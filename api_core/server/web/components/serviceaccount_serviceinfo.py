...
@xenSecureV1
class MtimeServiceAccountServiceInformationHandler(tornado.web.RequestHandler,
        MongoMixin):
                ...
                if len(app):
                    ...
                    if app[0]['apiId'] in [ 402020, 402021, 402022,402023]: # TODO: till here
                        if self.apiId == 402020:
                            ...
                            if serviceType == 6:
                                serviceInfoFindQ = self.serviceAccount.find(
                                                {
                                                    'serviceType':6,
                                ...
                                serviceInfoFind = []
                                async for i in serviceInfoFindQ:
                                    serviceInfoFind.append(i)
                                if len(serviceInfoFind):
                                    try:
                                        if serviceInfoFind[0]['serviceInfo'] == []:
                                            raise Exception
                                    except:
                                            code = 2390
                                            status = False
                                            ...
                                            raise Exception
                                    v = {
                                            'id':str(serviceInfoFind[0]['_id']),
                                            'activeDays': serviceInfoFind[0]['serviceInfo'][0]['activeDays'],
                                            'advanceDays': serviceInfoFind[0]['serviceInfo'][0]['advanceDays'],
                                            'adultCharge': serviceInfoFind[0]['serviceInfo'][0]['adultCharge'],
                                            'childCharge': serviceInfoFind[0]['serviceInfo'][0]['childCharge'],
                                            'maxCapacity': serviceInfoFind[0]['serviceInfo'][0]['maxCapacity']
                                        }
                                    result.append(v)
                                    code = 2000
                                    status = True
