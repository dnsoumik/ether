...
@xenSecureV1
class MtimeWebServiceVehicleTypeHandler(tornado.web.RequestHandler,
        MongoMixin):
            ...
            if len(profile):
                ...
                if len(app):
                    ...
                    if app[0]['apiId'] in [ 402021, 402022]: # TODO: till here
                        serVehicleFindQ = self.serviceVehicle.find({})
                        serVehicleFind = []
                        async for i in serVehicleFindQ:
                            serVehicleFind.append(i)
                        if len(serVehicleFind):
                            for res in serVehicleFind:
                                v = {
                                        'name':res['vehicleName']
                                    }
                                result.append(v)
                            code = 2000
                            status = True
            ...
            if len(profile):
                ...
                if len(app):
                    ...
                    if self.apiId in [ 402021, 402022]:
                        vehicleName = self.request.arguments.get('vehicleName')
                        ...
                        vehicleDesc = self.request.arguments.get('vehicleDesc')
                        code,message = Validate.i(
                                    vehicleDesc,
                                    'Vehicle Description',
                                    notEmpty = True,
                        ...
                        accConfId = yield self.serviceVehicle.insert(
                                            {
                                                'entityId':self.entityId,
                                                'disabled': False,
                                                'verified': False,
                        ...
                        code = 2000
                        message = "Vehicle Information has been submitted."
                        status = True
    ...
    def delete(self):
        ...
        try:
            try:
                ...
                code = 4850
                status = False
                message = "Invalid Service Vehicle ID"
                raise Exception
            # TODO: this need to be moved in a global class, from here
            profile = yield self.profile.find(
                            {
            ...
            if len(profile):
                ...
                if len(app):
                    if app[0]['apiId'] in [402022,402021]: # TODO: till here
                        serRemove = yield self.serviceVehicle.remove(
                                        {
                                            '_id': serId,
                                        }
                            )
