...
@xenSecureV1
class MtimeWebServiceStatusHandler(tornado.web.RequestHandler,
        MongoMixin):
                    ...
                    if app[0]['apiId'] in [ 402020, 402021, 402022]: # TODO: till here
                        if self.apiId == 402022:
                            ...
                            if status == None:
                                code = 8392
                                status = False
                                message = "Please select the status which you wish to set"
                                raise Exception
                            ...
                            serAccStatusUpdate = await self.serviceAccount.update_one(
                                                    ...
                                                    },
                                                    {
                                                        ...
                                                        '$push':{
                                                                    'activity':{
                                                                                    'task':"Service Account " + statusMsg,
                                                                                    'time':timeNow(),
                                                                                    'accountId':self.accountId,
                            ...
                            if serAccStatusUpdate.modified_count != None:
                                code = 2000
                                status = True
                                message = "Service Account Status has been set to " + statusMsg
                            else:
                                code = 7832
                                status = False
                                message = "Service Account Status could not be updated"
                        else:
                            code = 4003
                            self.set_status(401)
