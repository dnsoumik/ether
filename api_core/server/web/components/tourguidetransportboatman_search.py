...
@xenSecureV1
class MtimeTourGuideTransportBoatmanSearchHandler(tornado.web.RequestHandler,
        MongoMixin):
                ...
                if len(app):
                    ...
                    if app[0]['apiId'] in [ 402020, 402021, 402022,402023]: # TODO: till here
                        if self.apiId == 402020 or True:
                            if True:
                                try:
                                    serviceType = int(self.get_arguments('serviceType')[0])
                                    if serviceType not in [2,4,5]:
                                        raise Exception
                                except:
                                    code = 9982
                                    status = False
                                ...
                                if aRegex.isdigit():
                                    ownerNumberQ = self.serviceAccount.find(
                                                {
                                                    'serviceType':serviceType,
                                                    'contactInfo.providerNumber':int(aRegex),
                                                    'disabled':False
                                                }
                                            )
                                    ...
                                    for res in accList:
                                        ...
                                        if len(proFind):
                                            ...
                                            async for i in accFindQ:
                                                accFind.append(i)
                                            if len(accFind):
                                                registeredPhoneNum = accFind[0]['contact'][0]['value'] - 910000000000
                                            else:
                                                registeredPhoneNum = ''
                                        else:
                                            registeredPhoneNum = ''
                                        v = {
                                                    ...
                                                    'providerName':res['contactInfo'][0]['providerName'],
                                                    'providerNumber':res['contactInfo'][0]['providerNumber'],
                                                    'address':res['contactInfo'][0]['address'],
                                                    'providerEmail':res['contactInfo'][0]['providerEmail'],
                                                    'district':res['contactInfo'][0]['district'],
