...
@xenSecureV1
class MtimeScanCentersHandler(tornado.web.RequestHandler,
        MongoMixin):
    ...
    scanCenters = MongoMixin.serviceDb[
                    CONFIG['database'][1]['table'][49]['name']
                ]
            ...
            if len(profile):
                ...
                if len(app):
                    ...
                    if app[0]['apiId'] in [ 402020, 402022, 402021,402023]: # TODO: till here
                        if self.apiId in [402023,402022,402021]:
                            scanCentersFindQ = self.scanCenters.find({'disabled':False})
                            scanCentersFind = []
                            async for i in scanCentersFindQ:
                                i['_id'] = str(i['_id'])
                                scanCentersFind.append(i)
                            result = scanCentersFind
                            if len(result):
                                result.reverse()
                                code = 2000
                                status =True
                                message = "List of scan centers"
                            else:
                                code = 9302
                                status = False
                    ...
                    if app[0]['apiId'] in [ 402020, 402022, 402021,402023]: # TODO: till here
                        if self.apiId in [402021]:
                            ...
                            if location == None or location == "":
                                code = 9034
                                status = False
                                message = "Please submit valid location Name"
                                raise Exception
                            serUpdate = self.serviceAccount.update_one(
                                                                        {
                            ...
                            if serUpdate:
                                code = 2000
                                status = True
                                message = "Scan center location has been updated"
                            else:
                                code = 4004
                                status = False
                                message = "Scan center location could not be updated"
                                raise Exception
                        else:
                            code = 4003
                            self.set_status(401)
