...
@xenSecureV1
class MtimePlaceSearchAccomodationInfoHandler(tornado.web.RequestHandler,
        MongoMixin):
                    ...
                    if self.apiId in [ 402021, 402022, 402023]:
                        if self.apiId in [402022,402023]:
                            ...
                            async for i in serviceAccountFindQ:
                                serviceAccounts.append(i['_id'])
                            serviceAccountList = {"$in":serviceAccounts}
                            ...
                            if bookingId:
                                ...
                                if activityId != None:
                                    if activityId == 8:
                                        ...
                                        '''
                                        resQuery['$where'] = 'this.touristCount > 0 && (this.activity[this.activity.length - 1].id == 3 || this.activity[this.activity.length - 1].id == 1 ||\
                                                this.activity[this.activity.length - 1].id == 8) && this.bookType == 0'
                                    resQuery['serviceAccountId'] = serviceAccountList
                                    if f_state:
                                        resQ = self.touristBook.find(
                                        resQuery,
                                        limit=f_limit,
