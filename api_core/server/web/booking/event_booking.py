...
@xenSecureV1
class MtimeWebEventBookHandler(tornado.web.RequestHandler,
        MongoMixin):
                        ...
                        if self.apiId == 402020:
                            ...
                            if len(bookInfo):
                                for res in bookInfo:
                                    ...
                                    serviceDetails = []
                                    eventAccQ = self.serviceResource.find(
                                                    {
                                    ...
                                    async for i in eventAccQ:
                                        eventAcc.append(i)
                                    if len(eventAcc):
                                        serviceAccountQ = self.serviceAccount.find(
                                                            {
                                                                '_id':eventAcc[0]['serviceAccountId'],
                                                                'serviceType':8
                                                            }
                                                        )
                                        serviceAccount = []
                                        async for i in serviceAccountQ:
                                            serviceAccount.append(i)
                                        if len(serviceAccount):
                                            v = {
                                                    'title':eventAcc[0]['serviceInfo'][0]['title'],
                                                    'startDate':eventAcc[0]['serviceInfo'][0]['startDate'],
                                                    'endDate':eventAcc[0]['serviceInfo'][0]['endDate'],
                                                    'address':eventAcc[0]['serviceInfo'][0]['address'],
                                                    'owner':serviceAccount[0]['basicInfo'][0]['touristSpotName'],
                                                    'mobileNumber':serviceAccount[0]['basicInfo'][0]['mobileNumber'],
                                                    'paymentInfo':serviceAccount[0]['paymentInfo']
                                                }
                                            serviceDetails.append(v)
                                    x['serviceDetails'] = serviceDetails
                                    result.append(x)
                                result.reverse()
                                code = 2000
                                status = True
                            ...
                            if activityId in [-1,0,1,2,5,8]:
                                if activityId == 0:
                                    activityId = 1
                                    query = 'this.activity[this.activity.length - 1].id <= 1'
                                elif activityId == -1:
                                    query = 'this.activity[this.activity.length - 1].id > -1'
                                else:
                                    query = 'this.activity[this.activity.length - 1].id =='+str(activityId)
                            ...
                            if activityId in [0,1,8]:
                                ...
                                if len(bookInfo):
                                    for res in bookInfo:
                                        ...
                                        async for i in eventAccQ:
                                            eventAcc.append(i)
                                        for info in eventAcc:
                                            eventdetails.append(info)
                                            imgSample = {
            ...
            if len(profile):
                ...
                if len(app):
                    ...
                    if self.apiId in [ 402020, 402021]:
                        if self.apiId == 402020:
                            eventId = self.request.arguments.get('eventId')
                            if eventId == None:
                                code = 4560
                                status = False
                            ...
                            eventInfoQ = self.serviceResource.find(
                                        {
                                            '_id':eventId
                                        }
                                    )
                            eventInfo = []
                            async for i in eventInfoQ:
                                eventInfo.append(i)
                            if eventInfo == []:
                                code = 4680
                                status = False
                                ...
                                raise Exception
                            try:
                                bookTime = eventInfo[0]['serviceInfo'][0]['startDate']
                            except:
                                code = 2399
                                status = False
                            ...
                            eventProfile = eventInfo[0]['profileId']

                            eventAccQ = self.profile.find(
                                        {
                                            '_id':eventProfile,
                                        }
                                    )
                            ...
                            eventAccount = eventAcc[0]['accountId']
                            ...
                            payment = (numOfAdult * eventInfo[0]['serviceInfo'][0]['adultCharge']) \
                                    + (numOfChild * eventInfo[0]['serviceInfo'][0]['childCharge'])
