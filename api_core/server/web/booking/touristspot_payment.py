...
@xenSecureV1
class MtimeWebTouristSpotPaymentHandler(tornado.web.RequestHandler,
        MongoMixin):
                    ...
                    if self.apiId in [ 402020, 402021]:
                        if self.apiId == 402020:
                            ...
                            if not len(serAcc):
                                code = 2892
                                status = False
                                message = "Invalid tourist spot"
                                raise Exception

                            if serAcc[0].get('paymentInfo') == None:
                                code = 7837
                                status = False
                                message = "Online payment currently not supported for this tourist spot"
                                raise Exception

                            if serAcc[0]['paymentInfo'] == []:
                                code = 7837
                                status = False
                                message = "Online payment currently not supported for this tourist spot"
                                raise Exception

                            serAccUpiAddress = serAcc[0]['paymentInfo'][0]['upiId']
